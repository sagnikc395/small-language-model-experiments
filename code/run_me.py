...
def create_model(
    ...
    Returns:
        (Model, Value_to_Plot, Label_for_Plot)
    """
    # 1. Determine what we are varying (for the X-axis of plots)
    # We look for 'varying_param' in config, otherwise fallback to defaults
    param_val = conf.get("varying_param", None)
    param_label = conf.get("param_label", "Hyperparameter")

    # 2. Extract actual model type if 'arch' is a sweep name (e.g. 'linear_act')
    model_type = conf.get("model_type", arch)

    match model_type:
        case "linear":
            # Pass activation if present (default to identity)
            act = conf.get("activation", "identity")
            model = LinearRegressionModel(vocab_size, ctx, activation=act)
            # Default fallback if config didn't specify param info
        ...
        case "optimization":
            # Fixed architecture for optimizer sweep
            model = TransformerLM(
                vocab_size,
                ctx,
                embed_dim=128,
...
def save_plots(
    ...
    plt.figure()
    if isinstance(x_vals[0], str):  # Bar chart for categorical (e.g., activations)
...
def train_model(
    ...
    Returns: {'train_loss': [...], 'test_ll': [...], 'flops': [...]}
    """
    ...
    if context_len is not None:
        ...
        # Cast dataset to Sized to fix Pylance error
    ...
    for ep in range(epochs):
        ...
        # Optional: Print debug info only on first batch/epoch to reduce clutter
        # print(f"DEBUG: Starting Epoch {ep + 1}...")
...
def run_deliverable_1():
    ...
    with open(report_path, "w") as f:
        # Loop through Experiment Groups (e.g. 'linear_ctx', 'linear_act', 'mlp')
        for exp_name, configs in EXPERIMENT_CONFIG_DELIVERABLE_1.items():
            ...
            for conf in configs:
                ...
                # Create Model
                # 'exp_name' is passed just in case, but config usually dictates model_type
                ...
                if test_ll < best_ll:
                    best_ll = test_ll
                    # FIX: Passed arguments in correct order: context(3rd), context_len(4th), length(5th)
            ...
            print(f"    Best {exp_name} Test LL: {best_ll:.4f}")
            save_plots(exp_name, results_data, best_artifacts["curve"], REPORT_DIR)

            f.write(f"=== {exp_name.upper()} ===\n")
            f.write(f"Best Config: {best_artifacts['config']}\n")
            f.write(f"{best_artifacts['gen']}\n\n")

    print(f"Deliverable 1 Complete. Results in {REPORT_DIR}")
...
def run_deliverable_2():
    ...
    for task in EXPERIMENT_CONFIG_DELIVERABLE_2:
        ...
        for prompt in task["prompts"]:
            # Correct arguments for generate_words as well
